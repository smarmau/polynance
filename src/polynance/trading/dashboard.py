...
class TradingDashboard:
    ...
    def _make_header(self) -> Panel:
        ...
        # Signal time is at t=7.5 minutes (450 seconds)
        signal_time_sec = 7.5 * 60  # 450 seconds
        ...
        # Determine window phase and format countdowns
    ...
    def _make_slot_status_panel(self) -> Panel:
        ...
        Shows both the t=7.5 price (used for signal) and current price,
        plus whether a trade was opened.
        """
        table = Table(show_header=True, header_style="bold white", expand=True, box=None)
        table.add_column("Asset", width=5)
        table.add_column("@t7.5", justify="right", width=6)  # Price at signal time
        table.add_column("Now", justify="right", width=6)    # Current price
        table.add_column("Signal", width=5)
        ...
        for asset in self.sampler.assets:
            ...
            # Get price at t=7.5 (signal time)
            price_at_7_5 = None
            current_price = None
            ...
            # Determine what signal was at t=7.5
            signal_at_7_5 = None
            if price_at_7_5 is not None:
                if price_at_7_5 >= self.trader.bull_threshold:
                    signal_at_7_5 = "bull"
                elif price_at_7_5 <= self.trader.bear_threshold:
                    signal_at_7_5 = "bear"

            # Check if we have an open trade for this asset
            has_open_trade = asset in open_trades

            # Format t=7.5 price
            if price_at_7_5 is not None:
                p_color = "green" if price_at_7_5 >= 0.8 else "red" if price_at_7_5 <= 0.2 else "white"
            ...
            # Format signal (what happened at t=7.5)
            if signal_at_7_5 == "bull":
                signal_str = "[green]BULL[/green]"
            elif signal_at_7_5 == "bear":
                signal_str = "[red]BEAR[/red]"
            else:
                signal_str = "[dim]-[/dim]"
            ...
            if has_open_trade:
                ...
                trade_str = f"[green bold]OPEN[/green bold]"
            elif signal_at_7_5:
                # Had signal but no trade? Shouldn't happen
                trade_str = "[yellow]MISS?[/yellow]"
            else:
                trade_str = "[dim]SKIP[/dim]"

            table.add_row(asset, t75_str, now_str, signal_str, trade_str)

        return Panel(table, title="Window Status", border_style="white")
    ...
    def _make_footer(self) -> Panel:
        ...
        content.append(f">={self.trader.min_trajectory:.2f}", style="cyan")
        content.append("  |  ", style="dim")
